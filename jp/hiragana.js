...
class HiraganaHelper {
    ...
    static checkAnswer(userAnswer, correctAnswer) {
        console.log('INSIDE checkAnswer - userAnswer:', userAnswer, 'correctAnswer:', correctAnswer);
        
        // Convert both to lowercase for comparison
        const userNormalized = userAnswer.toLowerCase().trim();
        const correctNormalized = correctAnswer.toLowerCase().trim();
        
        console.log('AFTER normalize - userNormalized:', userNormalized, 'correctNormalized:', correctNormalized);
        
        // Special cases for common romaji variations
        const specialCases = {
            'wo': ['o'],
            'n': ['nn'],
            'tsu': ['tu'],
        ...
        if (userNormalized === correctNormalized) {
            console.log('DIRECT MATCH - returning true');
            return true;
        }
        
        // Check special cases
        if (specialCases[correctNormalized]) {
            const result = specialCases[correctNormalized].includes(userNormalized);
            console.log('SPECIAL CASE CHECK - result:', result);
            return result;
        }
